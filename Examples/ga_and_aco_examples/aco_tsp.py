...
from random import choices, random, uniform, choice
...
from ga_tsp import order_elements
...
class ACO_World(GA_World):
    ...
    def generate_a_tour(self, best=False) -> List[ACO_Link]:
        ...
        while unvisited_cities:
            ...
            # *start at city
            # look for the city with the strongest pheromone
            max_pheromone = 0
            current_link = None

            # denominator = 0
            # for link in [x for x in World.links if x.includes(current_city)]:
            #     if link.other_side(current_city) in unvisited_cities:
            #         denominator += pow(link.pheromone_level,alpha) * pow(link.length,beta)

            neighbor_list = [x for x in World.links if x.includes(current_city) and x.other_side(current_city) in unvisited_cities]
            percentage = list()
            for link in neighbor_list:
                if best:
                    if link.pheromone_level >= max_pheromone:
                        max_pheromone = link.pheromone_level
                        current_link = link
                else:
                    chance = int(100 * pow(link.pheromone_level,alpha) / pow(link.length,beta) ) #/ denominator)
                    print(f'Chance:{chance}')
                    percentage.append(chance)

            if not best:
                current_link = choices(neighbor_list, weights = percentage)

            (current_link.from_city, current_link.to_city) = (current_city, current_link.other_side(current_city))
            # - append link to tour
            tour.append(current_link)

            # - remove next city from unvisited cities
            unvisited_cities.remove(current_link.to_city)

            # - update current city
            current_city = current_link.to_city
        ...
        for link in World.links:
            if link.includes(start_city) and link.includes(current_city):
                final_link = link
                break
    ...
    def update_pheromone_levels(self, tour: List[ACO_Link]):
        ...
        The following is an outline of what I did. You don't have to follow that approach.
        """
        ...
        def max_increment(lnk):
            """ The maximum amount by which lnk's pheromone level may increase due to one tour. """
        ...
        #
        # I made it a function of how the current tour compares to the current best tour.
        # 'update_weight' puts a weight on how much a good tour is considered important.


        update_weight = gui_get('update_weight')

        #raw_increment = Q / tour_length
        raw_increment = (self.best_tour_length / tour_length) * (update_weight /10)

        print('raw:',raw_increment)
        for lnk in tour:
